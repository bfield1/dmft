    ...
    def __factory_from_dict__(cls, name, D):
        ...
        def convert_timedelta(t):
            if isinstance(t, dict):
                ...
                return ret
        if 'run_times' in D:
            # h5py seems to give results as np.int64 type, but timedelta only reads int type.
            D['run_times'] = [convert_timedelta({k: int(t[k]) for k in t}) for t in D['run_times']]
        if 'run_time_total' in D:
            # h5py seems to give results as np.int64 type, but timedelta only reads int type.
            D['run_time_total'] = {k: int(D['run_time_total'][k]) for k in D['run_time_total']}
